...
router.get("/read/:uid", function (req, res) {
  //localhost:5000/users/read/blue
  const uid = req.params.uid;
  const sql =
    'select *, date_format(regdate,"%Y-%m-%d %T") fmtdate, date_format(modidate,"%Y-%m-%d %T") fmtmodi from users where uid=?';
  db.get().query(sql, [uid], function (err, rows) {
    res.send(rows[0]);
  });
});
...
router.post("/update", function (req, res) {
  ...
  const uid = req.body.uid;
  const sql =
    "update users set uname=?,phone=?,address1=?,address2=?,modidate=now() where uid=?";
